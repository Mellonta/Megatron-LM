...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-router-load-balancing-type', nargs='+', type=str,
                       ...
                       default='aux_loss',
                       help='Determines the load balancing strategy for the router. "aux_loss" corresponds to the load balancing loss used in GShard and SwitchTransformer; "seq_aux_loss" corresponds to the load balancing loss used in DeepSeekV2, which computes the loss for each individual sample; "sinkhorn" corresponds to the balancing algorithm used in S-BASE; "quantile_balancing" (QB) uses dual coordinate descent on a per-expert bias and composes with the per-sequence aux loss (seq_aux_loss is applied whenever QB is active; disable it by setting --moe-aux-loss-coeff 0); "none" implies no load balancing. The default is "aux_loss".')
